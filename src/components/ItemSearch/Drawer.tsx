...
export default function Drawer({ children, isOpen, setIsOpen }: { children: any, isOpen: any, setIsOpen: any }) {
  const { handleFilter, setData }: any = useContext(FilterContext);
          ...
          <div >
            <div className='py-5 px-8 flex justify-between items-center bg-[#F5F7F8]'>
              <div className='flex gap-x-4 items-center'>
                <img className='h-5 w-5' src="assets/open-filter.svg" alt="open-filter" />
                <div>
                  <p className='text-2xl'>Set Parameters</p>
                  <p className='text-xs text-[#778FAB]'>9 parameters available</p>
                </div>
              </div>
              <div>
                <button onClick={() => setData([])} className='text-sm text-[#0C67A0] font-semibold'>Reset all</button>
              </div>
            </div>
            {children}
          </div>
